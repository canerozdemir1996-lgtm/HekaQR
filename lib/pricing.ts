  ...
  const body =
    params.locale === "tr"
      ? [
          "Merhaba,",
          "",
          "Kurumsal paket icin teklif talep ediyorum.",
          "",
          `Ad Soyad: ${params.name}`,
          `Sirket: ${params.company}`,
          ...
          summary,
          "",
          `Tahmini aylik: ${formatCurrency("tr", params.monthly)}`,
          `Tahmini yillik aylik karsilik: ${formatCurrency("tr", params.yearlyMonthly)}`,
          "",
          `Not: ${params.note || "-"}`,
        ].join("\n")
      : [
