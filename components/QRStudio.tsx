...
export default function QRStudio({ initialUrl = "", onSave, theme = "dark" }: {
  initialUrl?: string; onSave?: (cfg: QRStudioConfig) => Promise<void>; theme?: "dark" | "light"
}) {
  const isDark = theme === "dark";
  const [cfg, setCfg] = useState<QRStudioConfig>({ ...DEFAULT, url: initialUrl || DEFAULT.url });
  ...
  const p = useCallback(<K extends keyof QRStudioConfig>(k: K, v: QRStudioConfig[K]) => setCfg(prev => ({ ...prev, [k]: v })), []);

  const buildOptions = useCallback((c: QRStudioConfig, logoB64: string | null, size?: number) => {
        ...
        <div className="w-full lg:w-80 xl:w-96 space-y-3 shrink-0">
          ...
          <Section title="Arka Plan">
            ...
            <div>
              ...
              <Chips val={cfg.ecLevel} onChange={v => p("ecLevel", v as QRStudioConfig["ecLevel"])}
