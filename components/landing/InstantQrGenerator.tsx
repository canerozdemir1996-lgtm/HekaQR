...
export default function InstantQrGenerator() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 md:py-20">
      <div className="grid gap-8 rounded-[2.5rem] border border-violet-200 bg-white/85 p-6 shadow-xl shadow-violet-200/40 dark:border-violet-500/20 dark:bg-white/[0.04] dark:shadow-black/25 md:p-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
        <div>
          ...
          <h2 className="text-3xl font-black tracking-tight sm:text-4xl">Anında, ücretsiz QR kod oluşturun.</h2>
          <p className="mt-4 max-w-xl text-base font-semibold leading-7 text-slate-600 dark:text-slate-300">
            Bir bağlantı girin, QR kodunuzu görün ve PNG olarak indirin — kayıt olmadan. Dinamik yönlendirme, tarama
            raporu, logo ve tasarım şablonları gibi gelişmiş özellikler için ücretsiz hesap açmanız yeterli.
          </p>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <input
              ...
              className="h-13 flex-1 rounded-2xl border border-slate-200 bg-white px-5 py-3.5 text-sm font-semibold text-slate-900 outline-none transition focus:border-violet-400 dark:border-white/10 dark:bg-slate-950 dark:text-white"
            />
            <button
              type="button"
              onClick={download}
        ...
        <div className="flex flex-col items-center gap-4 rounded-[2rem] border border-slate-200 bg-slate-50 p-6 dark:border-white/10 dark:bg-slate-950/40">
          <div className="flex h-[220px] w-[220px] items-center justify-center overflow-hidden rounded-2xl bg-white shadow-inner" ref={mountRef} aria-label="Anlık QR kod önizlemesi">
