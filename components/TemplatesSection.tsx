  ...
  useEffect(() => {
    ...
    import("qr-code-styling").then(({ default: QRCodeStyling }) => {
      ...
      if (!qrRef.current) {
        ...
        const qr = new QRCodeStyling(opts as unknown);
...
function MiniQR({ style }: { style: QrStyle }) {
  ...
  useEffect(() => {
    ...
    import("qr-code-styling").then(({ default: QRCodeStyling }) => {
      ...
      const qr = new QRCodeStyling(buildOpts(cfg, null, 88) as unknown);
...
export function TemplatesSection({ isDark, onBack }: { isDark: boolean; onBack?: () => void }) {
  ...
  const exportPng = async () => {
    ...
    await (new Q(buildOpts(cfg, logoData) as unknown) as unknown as { download:(o:object)=>Promise<void> }).download({ name:"qrhub-template", extension:"png" });
  };
  const exportSvg = async () => {
    const { default: Q } = await import("qr-code-styling");
    await (new Q(buildOpts(cfg, logoData, 3000) as unknown) as unknown as { download:(o:object)=>Promise<void> }).download({ name:"qrhub-template", extension:"svg" });
  };
