...
import { CheckCircle2, FileCode2, FlaskConical, Loader2, Play, Search, TestTube2, X } from "lucide-react";
...
export default function AdminTestsClient({ catalog }: { catalog: AdminTestCatalogEntry[] }) {
  ...
  async function runTests(type: AdminTestCatalogEntry["type"], file?: string) {
    ...
    try {
      ...
      setResult({ ...data, ok: response.ok && data.ok, type, file: file ?? null, exitCode: data.exitCode ?? 1, output: data.output ?? data.error ?? "Çıktı alınamadı.", durationMs: data.durationMs ?? 0 });
    } catch (error) {
      setResult({ ok: false, type, file: file ?? null, exitCode: 1, output: error instanceof Error ? error.message : "Test çalıştırılamadı.", durationMs: 0 });
    } finally {
      setRunning(null);
    }
  }
  ...
  return <div className="space-y-6 px-4 py-8 sm:px-6">
    ...
    <div className={`flex flex-wrap gap-3 rounded-2xl border p-4 ${card}`}><button type="button" disabled={running !== null} onClick={() => void runTests("unit")} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white transition hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-50">{running === "unit" ? <Loader2 size={16} className="animate-spin" /> : <Play size={16} />}Tüm birim testlerini çalıştır</button><button type="button" disabled={running !== null} onClick={() => void runTests("e2e")} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-black text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50">{running === "e2e" ? <Loader2 size={16} className="animate-spin" /> : <Play size={16} />}Tüm uçtan uca testleri çalıştır</button>{running ? <span className={`self-center text-sm font-bold ${muted}`}>Testler çalışıyor, lütfen bekleyin…</span> : null}</div>
    ...
    {result ? <div className="fixed inset-0 z-[100000] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="test-result-title" onMouseDown={() => setResult(null)}><div className={`relative flex max-h-[85vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl border shadow-2xl ${isDark ? "border-white/10 bg-slate-950" : "border-slate-200 bg-white"}`} onMouseDown={(event) => event.stopPropagation()}><div className={`flex items-start justify-between gap-4 border-b p-5 ${isDark ? "border-white/10" : "border-slate-200"}`}><div><p className={`text-xs font-black uppercase tracking-widest ${result.ok ? "text-emerald-500" : "text-red-500"}`}>{result.ok ? "Başarılı" : "Başarısız"}</p><h2 id="test-result-title" className={`mt-1 text-xl font-black ${text}`}>{result.file ?? TYPE_LABELS[result.type]}</h2><p className={`mt-1 text-xs ${muted}`}>Çıkış kodu: {result.exitCode} · Süre: {(result.durationMs / 1000).toFixed(1)} saniye</p></div><button type="button" onClick={() => setResult(null)} aria-label="Sonuç penceresini kapat" className={`rounded-xl p-2 ${isDark ? "hover:bg-white/10" : "hover:bg-slate-100"}`}><X size={20} /></button></div><pre className={`min-h-0 flex-1 overflow-auto whitespace-pre-wrap break-words p-5 font-mono text-xs leading-6 ${isDark ? "bg-black/30 text-slate-300" : "bg-slate-50 text-slate-700"}`}>{result.output}</pre><div className={`border-t p-4 text-right ${isDark ? "border-white/10" : "border-slate-200"}`}><button type="button" onClick={() => setResult(null)} className="rounded-xl bg-violet-600 px-5 py-2.5 text-sm font-black text-white hover:bg-violet-500">Kapat</button></div></div></div> : null}
  </div>;
}
