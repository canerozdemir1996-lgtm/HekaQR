...
export default function AdminTestsClient({ catalog }: { catalog: AdminTestCatalogEntry[] }) {
  ...
  return <div className="space-y-6 px-4 py-8 sm:px-6">
    <div><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-violet-500/15 text-violet-500"><FlaskConical size={21} /></div><div><h1 className={`text-xl font-black ${text}`}>Tests</h1><p className={`text-sm ${muted}`}>Repository içindeki otomatik test envanteri.</p></div></div></div>
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[{ label: "Test Dosyası", value: catalog.length, icon: FileCode2 }, { label: "Toplam Senaryo", value: totalTests, icon: TestTube2 }, { label: "Unit Test", value: unitTests, icon: CheckCircle2 }, { label: "E2E Test", value: e2eTests, icon: FlaskConical }].map((item) => <div key={item.label} className={`rounded-2xl border p-5 ${card}`}><item.icon size={18} className="text-violet-500" /><p className={`mt-4 text-2xl font-black ${text}`}>{item.value}</p><p className={`mt-1 text-xs font-bold uppercase tracking-wider ${muted}`}>{item.label}</p></div>)}</div>
    <div className={`rounded-2xl border p-4 ${card}`}><div className="flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><Search size={16} className={`absolute left-3 top-1/2 -translate-y-1/2 ${muted}`} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Dosya veya test senaryosu ara" className={`h-11 w-full rounded-xl border bg-transparent pl-10 pr-3 text-sm font-semibold outline-none focus:border-violet-500 ${isDark ? "border-white/10 text-white placeholder:text-slate-600" : "border-slate-200 text-slate-900 placeholder:text-slate-400"}`} /></label><div className="flex gap-2">{(["all", "unit", "e2e"] as Filter[]).map((value) => <button key={value} type="button" onClick={() => setFilter(value)} className={`rounded-xl px-4 py-2 text-xs font-black uppercase transition ${filter === value ? "bg-violet-600 text-white" : isDark ? "bg-white/5 text-slate-400 hover:text-white" : "bg-slate-100 text-slate-600 hover:text-slate-900"}`}>{value === "all" ? "Tümü" : value}</button>)}</div></div></div>
    <div className="space-y-4">{rows.map((entry) => <section key={entry.file} className={`overflow-hidden rounded-2xl border ${card}`}><div className={`flex flex-wrap items-center justify-between gap-3 border-b px-5 py-4 ${isDark ? "border-white/10" : "border-slate-200"}`}><div className="min-w-0"><h2 className={`truncate font-mono text-sm font-black ${text}`}>{entry.file}</h2><p className={`mt-1 text-xs ${muted}`}>{entry.tests.length} test senaryosu</p></div><span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase ${entry.type === "e2e" ? "bg-blue-500/15 text-blue-500" : "bg-emerald-500/15 text-emerald-500"}`}>{entry.type}</span></div><ol className={`divide-y ${isDark ? "divide-white/10" : "divide-slate-100"}`}>{entry.tests.map((name, index) => <li key={`${entry.file}-${name}`} className="flex gap-3 px-5 py-3"><span className={`font-mono text-xs ${muted}`}>{String(index + 1).padStart(2, "0")}</span><span className={`text-sm font-semibold ${text}`}>{name}</span></li>)}</ol></section>)}{rows.length === 0 ? <div className={`rounded-2xl border border-dashed py-16 text-center ${isDark ? "border-white/10" : "border-slate-200"}`}><Search size={24} className={`mx-auto ${muted}`} /><p className={`mt-3 text-sm font-bold ${muted}`}>Aramanızla eşleşen test bulunamadı.</p></div> : null}</div>
    <p className={`text-xs leading-5 ${muted}`}>Katalog build öncesinde otomatik yenilenir. Unit testler <code className="font-mono">npm test</code>, E2E testler <code className="font-mono">npm run test:e2e</code> ile çalıştırılır.</p>
  </div>;
}
