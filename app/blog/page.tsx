...
const blogMetadata = buildPageMetadata({
...
export default function BlogPage() {
  return <main className="min-h-screen bg-slate-50 text-slate-950 dark:bg-[#030712] dark:text-white">
    <JsonLd data={{ "@context": "https://schema.org", "@graph": [buildBreadcrumbListSchema([{ name: "Ana Sayfa", path: "/" }, { name: "Blog", path: "/blog" }]), { "@type": "CollectionPage", name: "QR Publish Blog", url: "https://qrpublish.com/blog", mainEntity: { "@type": "ItemList", itemListElement: blogPosts.map((post, index) => ({ "@type": "ListItem", position: index + 1, url: `https://qrpublish.com/blog/${post.slug}`, name: post.title })) } }] }} />
    <header className="border-b border-slate-200 bg-white/90 dark:border-white/10 dark:bg-slate-950/80"><div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-5 sm:px-6"><Link href="/"><BrandLogo className="w-[150px]" /></Link><nav className="flex gap-5 text-sm font-bold"><Link href="/qr-kod-olusturucu" className="hover:text-violet-600">QR Oluştur</Link><Link href="/blog/hakkinda" className="hover:text-violet-600">Hakkında</Link><Link href="/blog/rss.xml" className="hover:text-violet-600">RSS</Link></nav></div></header>
