...
export default function BookingPageClient({ slug, qrId, title, config }: Props) {
  ...
  async function submit() {
    setError("");
    setDone("");
    if (!selectedDate || !selectedTime) return setError("Lütfen tarih ve saat seçin.");
    if (!name.trim()) return setError("Ad soyad zorunlu.");
    if (!email.trim() && !phone.trim()) return setError("E-posta veya telefon girin.");
    setLoading(true);
    try {
      const res = await fetch("/api/v1/bookings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slug, qr_id: qrId, appointment_date: selectedDate, appointment_time: selectedTime, customer_name: name, customer_email: email, customer_phone: phone, note }),
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(typeof json.error === "string" ? json.error : "Rezervasyon alınamadı.");
      setDone(json.message || config.successMessage);
      setName(""); setEmail(""); setPhone(""); setNote("");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Rezervasyon alınamadı.");
    } finally {
      setLoading(false);
    }
  }
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-950 p-4 text-white">
        <div className="max-w-sm rounded-3xl border border-white/10 bg-white/5 p-6 text-center">
          ...
          <h1 className="mt-4 text-xl font-black">Rezervasyon kapalı</h1>
          <p className="mt-2 text-sm text-slate-300">Bu QR için randevu alımı şu anda aktif değil.</p>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#ecfeff,#f8fafc)] px-4 py-5 text-slate-950">
      <div className="mx-auto max-w-4xl">
        <section className="rounded-[2rem] bg-slate-950 p-5 text-white shadow-2xl shadow-cyan-900/20 sm:p-7">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-300">{config.serviceType || "Rezervasyon"}</p>
          ...
          <div className="mt-5 grid gap-2 text-sm font-bold sm:grid-cols-2">
            <span className="rounded-2xl bg-white/10 px-3 py-2"><MapPin size={15} className="mr-1 inline" />{config.location || config.onlineUrl || "Lokasyon belirtilmedi"}</span>
            <span className="rounded-2xl bg-white/10 px-3 py-2">{config.durationMinutes} dk · Kontenjan {config.capacity} · {config.timezone}</span>
          </div>
        </section>
        ...
        <section className="mt-4 rounded-[2rem] border border-slate-200 bg-white p-4 shadow-xl shadow-slate-200/60 sm:p-6">
          ...
          <div className="grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
            <div>
              <p className="mb-2 text-xs font-black uppercase tracking-wider text-slate-500">Tarih</p>
              <div className="grid max-h-72 gap-2 overflow-y-auto pr-1">
                {days.length === 0 ? <p className="text-sm font-semibold text-slate-500">Tarih aralığı tanımlanmamış.</p> : days.map(day => (
                  <button key={day} onClick={() => setSelectedDate(day)} className={`rounded-2xl border px-3 py-3 text-left text-sm font-black ${selectedDate === day ? "border-cyan-500 bg-cyan-50 text-cyan-800" : "border-slate-200 bg-white text-slate-700"}`}>
                    {new Date(`${day}T00:00:00`).toLocaleDateString("tr-TR", { weekday: "long", day: "2-digit", month: "long" })}
                  </button>
                ))}
              </div>
            </div>
            <div className="space-y-4">
              <div>
                <p className="mb-2 text-xs font-black uppercase tracking-wider text-slate-500">Saat</p>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <input value={name} onChange={e => setName(e.target.value)} placeholder="Ad soyad" className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold outline-none focus:border-cyan-500" />
                <input value={email} onChange={e => setEmail(e.target.value)} placeholder="E-posta" className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold outline-none focus:border-cyan-500" />
                <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Telefon" className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold outline-none focus:border-cyan-500" />
                <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Not (opsiyonel)" rows={3} className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold outline-none focus:border-cyan-500 sm:col-span-2" />
              </div>
              <button disabled={loading} onClick={submit} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3.5 text-sm font-black text-white shadow-lg shadow-slate-900/15 disabled:opacity-50">
                {loading ? <Loader2 className="animate-spin" size={16} /> : <Send size={16} />} Rezervasyon Talebi Gönder
              </button>
            </div>
          </div>
        </section>
